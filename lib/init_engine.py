...
def upload_via_pyspark(spark, input_csv, output_tb):
    ...
    ####TODO
    In terminal: 
    spark-submit --executor-memory 14g --packages com.databricks:spark-csv_2.10:1.4.0 rule_engine/lib/bulk_upload_process_2db.py -v "grn" -y "2019"
    python rule_engine/lib/init_engine.py -v "iqvia" -y "2019"
...
def run_GRN(spark, config):
    ...
    from lib.GRN_qc_engine import GRN_qc_engine
    GRN = GRN_qc_engine()
...
def run_COTA(spark, config):
    ...
    from lib.COTA_qc_engine import COTA_qc_engine
    COTA = COTA_qc_engine()
...
def run_Base(config):
    ...
    Base.init(config["refresh_dt"], config["folder"], config["project"], config["rules"])
    vendor_data = Base.read_into_pandas()
    for filename in vendor_data:
        pass
        #upload_to_hive(spark, vendor_data[filename], filename + "_RAW", config["database"])
    result = Base.run_engine( (vendor_data), config["rule_sequence"])
    
    output_loc = config["project"] + "/raw_qc/" + config["folder"] + "_" + re.sub('[-/]', '', config["refresh_dt"]) + "/"
    for filename in result:
        #result[filename].coalesce(1).write.format("com.databricks.spark.csv").option("header", "true").save(output_loc  + filename + ".txt")
        #result[filename].to_csv(output_loc + filename + ".txt", sep="|", index=False)
        #upload_to_hive(spark, result[filename], filename + "_QC", config["database"])
        pass
...
if __name__ == "__main__":
    ...
    for vendor in run_list:
        ...
        if vendor == "grn_mm_old":
            run_GRN(config["GRN"])
        elif vendor == "cota":
            run_COTA(config["COTA"])
        elif vendor.upper() in config:
            run_Base(config[vendor.upper()])
        else:
